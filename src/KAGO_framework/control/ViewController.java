...
public class ViewController implements ActionListener, KeyListener, MouseListener, MouseMotionListener {
    ...
    public ViewController(boolean isScenarioMode) {
        ...
        if (Config.INFO_MESSAGES)
            System.out.println("     > Es wird nun einmalig die Methode startProgram von dem ProgramController-Objekt aufgerufen. (außer im Szenario-Modus!)");
        if (Config.INFO_MESSAGES)
    ...
    private void createWindow() {
        ...
        if (Config.INFO_MESSAGES)
            System.out.println("  > ViewController: Fenster eingerichtet. Startszene (Index: 0) angelegt.");
    }
    ...
    public void showScene(int index) {
        ...
        if (index < scenes.size()) {
            ...
            if (Config.INFO_MESSAGES)
                System.out.println("  > ViewController: Fehler: Eine Szene mit dem Index " + index + " existiert nicht.");
        }
    }
